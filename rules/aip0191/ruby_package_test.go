...
func TestRubyPackage(t *testing.T) {
	for _, test := range []struct {
		...
		{"InvalidBetaChannel", `Google::Example::V1Beta`, testutils.Problems{{
			Suggestion: fmt.Sprintf("option ruby_package = %q;", `Google::Example::V1beta`),
		}}},
		{"InvalidTemplate", `Google::Example::V1Main`, testutils.Problems{{
			Suggestion: fmt.Sprintf("option ruby_package = %q;", `Google::Example::V1main`),
		}}},
		{"InvalidPointRelease", `Google::Example::V1P1Beta1`, testutils.Problems{{
			Suggestion: fmt.Sprintf("option ruby_package = %q;", `Google::Example::V1p1beta1`),
		}}},
