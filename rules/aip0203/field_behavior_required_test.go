...
func TestFieldBehaviorRequired_NestedMessages_SingleFile(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			it := f.Services()[0].Methods()[0].Input()
			nestedField := it.Fields()[0].GetMessageType().Fields()[0]
...
func TestFieldBehaviorRequired_NestedMessages_MultipleFile(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			f := ds["service.proto"]
			it := f.Services()[0].Methods()[0].Input()
			fd := it.Fields()[0].GetMessageType().Fields()[0]
			...
			if tc.problems != nil {
				want := "resource.proto"
				if got := fd.ParentFile().Name(); got != want {
