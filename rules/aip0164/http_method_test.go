...
func TestHttpMethod(t *testing.T) {
	...
	tests := []struct {
		...
		{"Valid", httpPost, "UndeleteBook", ""},
		{"Invalid", httpDelete, "UndeleteBook", "HTTP POST"},
		{"Irrelevant", httpDelete, "AcquireBook", ""},
	}
	...
	for _, test := range tests {
		t.Run(test.testName, func(t *testing.T) {
			...
			service, err := builder.NewService("Library").AddMethod(builder.NewMethod(test.methodName,
				builder.RpcTypeMessage(builder.NewMessage("UndeleteBookRequest"), false),
