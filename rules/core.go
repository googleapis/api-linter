...
func registerRuleWithVisitor(metadata ruleMetadata, visitor protoVisitor) {
	r := ruleBase{
		metadata: metadata,
		visitor:  visitor,
	}
...
func (r ruleBase) Lint(req lint.Request) (lint.Response, error) {
	...
	if r.visitor != nil {
		return r.visitor.Walk(req)
	}
	return lint.Response{}, nil
}

type protoVisitor interface {
	Walk(req lint.Request) (lint.Response, error)
}
...
func (v *simpleVisitor) Walk(req lint.Request) (lint.Response, error) {
	v.ctx = req.Context()
...
func (v *simpleVisitor) checkTopLevelEnums(f protoreflect.FileDescriptor) {
	if v.EnumCheck != nil {
		for i := 0; i < f.Enums().Len(); i++ {
			v.addProblems(v.EnumCheck(f.Enums().Get(i), v.ctx)...)
		}
	}
}
...
func (v *simpleVisitor) checkService(n protoreflect.ServiceDescriptor) {
	if v.ServiceCheck != nil {
		v.addProblems(v.ServiceCheck(n, v.ctx)...)
	}
}

func (v *simpleVisitor) checkMethod(m protoreflect.MethodDescriptor) {
	if v.MethodCheck != nil {
