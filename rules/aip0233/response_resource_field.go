...
var responseResourceField = &lint.MessageRule{
	Name:   lint.NewRuleName(233, "response-resource-field"),
	...
	LintMessage: func(m protoreflect.MessageDescriptor) []lint.Problem {
		...
		resourceMsgName := pluralize.NewClient().Singular(strings.TrimPrefix(strings.TrimSuffix(m.Name(), "Response"), "BatchCreate"))

		for _, fieldDesc := range m.Fields() {
			if msgDesc := fieldDesc.GetMessageType(); msgDesc != nil && msgDesc.Name() == resourceMsgName {
				if !fieldDesc.IsRepeated() {
					return []lint.Problem{{
						Message:    fmt.Sprintf("The %q type field on Batch Create Response message should be repeated", msgDesc.Name()),
